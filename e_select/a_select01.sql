...
select: 선택하다 
 cf) 데이터베이스는 어떻게 보다 무엇을 가지고 오는지가 중요
    ...
    2.from테이블명: 어떠 테이블에서 데이터를 가져올지결정 
    3.where 조건 : 특정  조건에 맞는 데이터만 선택 
    4.group by 그룹화할 컬럼명 : 특정 열을 기준으로 그룹화 
    5.having 그룹조건 :그룹화 한 이후의 조건을 지정 
    6.order by 정렬 컬럼 : 결과를 특정컬럼의 순서로 정렬
    7.limit 컬럼수 제한 :  반환할 컬럼(행)의 수를 제한 
    cf)
*/
use `kroea_db`;
...
select * from members
where
	name like 'J___'; #Jase,jamr 이렇게 J로 시작하는 네글자 이름 
